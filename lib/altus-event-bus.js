...
// sessionId → { clientId, createdAt }
const sessionRegistry = new Map();

export function registerSession(sessionId, clientId) {
  sessionRegistry.set(sessionId, { clientId, createdAt: Date.now() });
}

export function isSessionRegistered(sessionId, clientId) {
  const entry = sessionRegistry.get(sessionId);
  if (!entry) return false;
  // Sessions expire after 1 hour to prevent orphaned session hijacking
