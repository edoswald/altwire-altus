...
export async function initSchema() {
  ...
  try {
    ...
    await client.query(`
      CREATE TABLE IF NOT EXISTS altus_weekly_brief_batches (
        id              SERIAL PRIMARY KEY,
...
/**
 * Return the run_at of the most recent *successful* ingest run (errors = 0),
 * used to compute the incremental `after` window for daily syncs.
 * Returns null when there has never been a clean run.
 */
export async function getLastSuccessfulIngestRun() {
  try {
    const { rows } = await pool.query(
      `SELECT run_at FROM altus_ingest_log
        WHERE errors = 0
    ...
    logger.warn('getLastSuccessfulIngestRun failed', { error: err.message });
    return null;
  }
}

/**
 * Return the wp_ids of every indexed gallery. Used by recent/incremental
 * ingest to include galleries (which carry no modified timestamp from the WP
 * endpoint) only when they are brand-new to the archive.
 *
 * @returns {Promise<number[]>}
 */
export async function getIndexedGalleryIds() {
  try {
    const { rows } = await pool.query(
      `SELECT wp_id FROM altus_content WHERE content_type = 'gallery'`
    );
    return rows.map((r) => Number(r.wp_id));
  } catch (err) {
    logger.warn('getIndexedGalleryIds failed', { error: err.message });
    return [];
  }
}
...
// Re-export mountaineering schema init so index.js can include it in startup Promise.all
export { initMountaineeringSchema } from '../handlers/altus-mountaineering.js';
