...
// Module-level singleton — one client instance per process
...
/**
 * A gallery has "synthesizable material" when there is a non-empty description
 * or at least one image (within the first 20) with a non-empty alt/caption.
 * Galleries with none of that would only ever yield a restatement of title +
 * count from Claude — something the deterministic fallback does for free.
 */
export function hasSynthesizableMaterial(gallery) {
  if ((gallery.description ?? '').trim()) return true;
  const images = Array.isArray(gallery.images) ? gallery.images.slice(0, 20) : [];
  return images.some((img) => Boolean((img?.alt ?? '').trim() || (img?.caption ?? '').trim()));
}

/**
 * Build the Messages request params used to synthesize a gallery description.
 * Shared by synthesizeGallery() and the batch-synthesis path so both produce
 * byte-identical requests (and benefit from the cached system prompt).
 */
export function buildGallerySynthesisRequest(gallery) {
...
export async function synthesizeGallery(gallery) {
  ...
  // No Claude call for galleries with nothing to describe — the fallback is
  // the exact restatement the model would be forced to produce anyway.
  ...
  try {
    ...
    const response = await client.messages.create(buildGallerySynthesisRequest(gallery));

    const text = response.content.find((b) => b.type === 'text')?.text ?? '';
    logAiUsage('synthesize_gallery', response.model, response.usage).catch(() => {});
...
export async function synthesizeCoverageAssessment(subject, contextLines) {
  ...
  try {
    ...
    const response = await client.messages.create({
      ...
      max_tokens: 200,
      temperature: 0,
      system: withCachedSystem(`You are an editorial assistant for AltWire, an alternative music publication.
...
Do not use bullet points. Write 2-4 sentences maximum.`),
    ...
    const text = response.content.find((b) => b.type === 'text')?.text ?? '';
    logAiUsage('analyze_coverage_gaps', response.model, response.usage).catch(() => {});
...
export async function synthesizePitches(opportunities) {
  ...
  try {
    ...
    const response = await client.messages.create({
      ...
      max_tokens: 400,
      temperature: 0.3,
      system: withCachedSystem(`You are an editorial strategist for AltWire, an alternative music publication. Generate ${pitchCount} concise editorial pitches based on search demand data. Each pitch should suggest a specific article angle, not just restate the query. Be actionable and specific.`),
      messages: [{
        role: 'user',
