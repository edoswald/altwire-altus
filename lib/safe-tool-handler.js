...
/**
 * Coerce a value to a finite integer, or null. Session IDs may be UUID strings
 * (non-numeric), which Number() turns into NaN — and the altus_events.session_id
 * column is INTEGER, so an unguarded NaN fails the insert. Return null instead.
 * @param {unknown} value
 * @returns {number|null}
 */
...
let _logAltusEvent = undefined;
async function getLogAltusEvent() {
  if (_logAltusEvent !== undefined) return _logAltusEvent;
  try {
  ...
  return async (params) => {
    ...
    try {
      ...
      getLogAltusEvent().then(logFn => {
        if (logFn) logFn('tool_call', { tool_name: toolName, session_id: toIntOrNull(sessionId), duration_ms: durationMs }).catch((err) => logger.warn('altus_event_log insert failed', { tool_name: toolName, error: err.message }));
      }).catch((err) => logger.warn('altus_event_log: getLogAltusEvent failed', { error: err.message }));
      ...
      // Async log — do not block response
      getLogAltusEvent().then(logFn => {
        if (logFn) logFn('tool_error', { tool_name: toolName, session_id: toIntOrNull(sessionId), error_message: err.message, duration_ms: durationMs }).catch((logErr) => logger.warn('altus_event_log insert failed', { tool_name: toolName, error: logErr.message }));
      }).catch((err) => logger.warn('altus_event_log: getLogAltusEvent failed', { error: err.message }));
