  ...
  return async (params) => {
    ...
    try {
      ...
      getLogAltusEvent().then(logFn => {
        if (logFn) logFn('tool_call', { tool_name: toolName, session_id: sessionId ? Number(sessionId) : null, duration_ms: durationMs }).catch(() => {});
      });
      ...
      // Async log — do not block response
      getLogAltusEvent().then(logFn => {
        if (logFn) logFn('tool_error', { tool_name: toolName, session_id: sessionId ? Number(sessionId) : null, error_message: err.message, duration_ms: durationMs }).catch(() => {});
      });
