 ...
 * buildAgentTokenHeader() — Shared X-Agent-Token auth for plugin action APIs
 ...
 * getSeoState()        — Read current cirrusly-seo SEO state for a WP object
 * updateSeoFields()    — Update allowlisted cirrusly-seo SEO fields
 */
...
/**
 * Build the shared agent-token header for plugin action APIs.
 *
 * @param {'read'|'write'} scope
 * @returns {string}
 */
export function buildAgentTokenHeader(scope = 'read') {
  const scoped = scope === 'write'
    ? (process.env.ALTWIRE_WP_AGENT_WRITE_TOKEN ?? '')
    : (process.env.ALTWIRE_WP_AGENT_READ_TOKEN ?? '');
  const fallback = process.env.ALTWIRE_WP_AGENT_TOKEN ?? '';
...
async function pluginFetch(path, { method = 'GET', scope = 'read', body = null, searchParams = null } = {}) {
  const token = buildAgentTokenHeader(scope);
  if (!token) {
    throw new Error(`Missing AltWire WP agent ${scope} token`);
  }
  ...
  if (!res.ok) {
    const err = new Error(
      data?.message
        ? `WP plugin fetch failed: ${res.status} ${data.message}`
        : `WP plugin fetch failed: ${res.status} ${url}`
    );
    err.status = res.status;
    err.code = data?.code || 'wp_plugin_fetch_failed';
...
/**
 * Read current SEO state from the cirrusly-seo action API.
 *
 * @param {{ objectType: string, objectId?: number }} params
 * @returns {Promise<object>}
 */
export async function getSeoState({ objectType, objectId }) {
  return pluginFetch('/cirrusly/v1/seo/state', {
    scope: 'read',
    searchParams: {
...
/**
 * Update allowlisted SEO fields through the cirrusly-seo action API.
 *
 * @param {{ objectType: string, objectId?: number, fields: Record<string, unknown>, reason?: string }} params
 * @returns {Promise<object>}
 */
export async function updateSeoFields({ objectType, objectId, fields, reason }) {
  return pluginFetch('/cirrusly/v1/seo/update-fields', {
    method: 'POST',
    scope: 'write',
    body: {
      object_type: objectType,
...
export async function fetchTaxonomies() {
  ...
  // Paginate through all categories using WordPress total-page headers
  let page = 1;
  let totalPages = 1;
  while (page <= totalPages) {
    const { data: cats, totalPages: headerTotalPages } = await wpFetch(`/wp/v2/categories?per_page=100&page=${page}`);
    totalPages = Math.max(1, headerTotalPages);
    for (const c of cats) categoryCache.set(c.id, c.name);
    if (!Array.isArray(cats) || page >= totalPages) break;
    page++;
  }

  // Paginate through all tags using WordPress total-page headers
  page = 1;
  totalPages = 1;
  while (page <= totalPages) {
    const { data: tags, totalPages: headerTotalPages } = await wpFetch(`/wp/v2/tags?per_page=100&page=${page}`);
    totalPages = Math.max(1, headerTotalPages);
    for (const t of tags) tagCache.set(t.id, t.name);
    if (!Array.isArray(tags) || page >= totalPages) break;
    page++;
  }
...
export async function fetchPosts({ categoryCache, tagCache }, afterDate = null) {
  ...
  let page = 1;
  const perPage = 100;
  const MAX_PAGES = 100; // guard against runaway pagination
  ...
  while (true) {
    ...
    if (afterDate !== null) {
      endpoint += `&after=${encodeURIComponent(afterDate)}&modified_after=${encodeURIComponent(afterDate)}`;
    }
    const { data: items } = await wpFetch(endpoint);
    ...
    if (items.length < perPage || page >= MAX_PAGES) break;
    page++;
  }
...
export async function fetchGalleries() {
  ...
  while (true) {
    const { data: items } = await wpFetch(`/altus/v1/galleries?page=${page}&per_page=${perPage}`);
    all.push(...items);
