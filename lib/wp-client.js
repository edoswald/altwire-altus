...
export async function fetchTaxonomies() {
  ...
  // Paginate through all categories (WP caps at 100 per page)
  let page = 1;
  let totalPages = 1;
  while (page <= totalPages) {
    const cats = await wpFetch(`/wp/v2/categories?per_page=100&page=${page}`);
    for (const c of cats) categoryCache.set(c.id, c.name);
    // WP sets X-WP-TotalPages header when paginated; if response is an array and page ≥ totalPages, we're done
    if (!Array.isArray(cats) || cats.length < 100 || page >= totalPages) break;
    totalPages = page + 1; // assume more pages exist until proven otherwise
    page++;
  }
  ...
  totalPages = 1;
  while (page <= totalPages) {
    const tags = await wpFetch(`/wp/v2/tags?per_page=100&page=${page}`);
    for (const t of tags) tagCache.set(t.id, t.name);
    if (!Array.isArray(tags) || tags.length < 100 || page >= totalPages) break;
    totalPages = page + 1;
    page++;
  }
