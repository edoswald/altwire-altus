...
async function generateAnthropic({ toolName, system, prompt, maxTokens, webSearch, jsonMode }) {
  ...
  const requestParams = {
    ...
    // Cache the system prompt — it is stable across the many generation calls the
    // writer pipeline makes, so it should be written once and read thereafter
    // rather than re-billed at full input price on every article.
    system: withCachedSystem(effectiveSystem),
