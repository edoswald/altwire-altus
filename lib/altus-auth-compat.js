...
// Token issuer — identifies this service as the minter of a session token.
// Altus and Nimbus share HAL_SESSION_SECRET, so a token signed by one verifies
// against the other. The `iss` claim lets each service tell its own tokens
// apart from a peer's and decide whether to trust the embedded scope.
const TOKEN_ISSUER = 'altus';
...
/**
 * Re-derive the canonical local scope for a web identity by name, ignoring any
 * scope claim carried in the token. Applied to tokens minted by a peer service
 * so a scope stamped elsewhere cannot cross the boundary. Identities with no
 * matching HAL_KEY_*_WEB entry default to 'guest'.
 */
export function localScopeForWebIdentity(identity, env = process.env) {
  if (!identity || !identity.name) return 'guest';
  for (const { name, scope } of buildWebKeyRegistry(env)) {
    if (name === identity.name) return scope;
  }
  return 'guest';
}
...
export function signHalWebSessionToken(identity, env = process.env) {
  ...
  const payload = toBase64url(JSON.stringify({ ...identity, iss: TOKEN_ISSUER, exp }));
...
export function verifyHalWebSessionToken(token, env = process.env) {
  ...
  try {
    ...
    // Scope trust boundary: honor the embedded scope only for tokens this
    // service minted. Peer-issued tokens (e.g. Nimbus) get their scope
    // re-derived from local identity policy so a foreign scope cannot leak in.
    const selfIssued = decoded.iss === TOKEN_ISSUER;
    const scope = selfIssued
      ? (decoded.scope ?? 'full')
      : localScopeForWebIdentity({ name: decoded.name }, env);
