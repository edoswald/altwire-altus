...
// The synthesis is generated by an LLM tool call. The schema declares these
// fields as arrays, but the model occasionally emits a non-array (e.g. a string
// or single object). Coerce them to arrays at the source so every downstream
// consumer (digest mailer, action-item proposer, recordSynthesisFeatures) can
// safely call array methods. A bare object is wrapped; anything else becomes [].
const SYNTHESIS_ARRAY_FIELDS = [
  'top_dual_winners',
  'underperforming_in_search',
  'opportunity_alignments',
  'content_gaps',
...
function normalizeSynthesis(synthesis) {
  ...
  for (const field of SYNTHESIS_ARRAY_FIELDS) {
...
export async function getCombinedAnalytics(opts = {}) {
  ...
  if (doSynth && (matomoConfigured || gscConfigured)) {
    ...
    synthesis = normalizeSynthesis(await synthesize(synthPayload));
  }
